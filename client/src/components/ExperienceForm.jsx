...
const ExperienceForm = ({data, onChange}) => {
    ...
    const generatDescription = async (index) =>{
        ...
        const prompt = `enhance this job description ${experience.description} for position of ${experience.company}.`
                ...
                {data.map((experience, index)=> (
                    <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                        ...
                        <label className='flex items-center gap-2'>
                            <input type="checkbox" checked={experience.is_current || false} onChange={(e)=> {updateExperience(index,"is_current", e.target.checked ? true : false);}} className='rounded border-gray-300 text-blue-600 focus:ring focus:ring-blue-500 outline-none'/>
                        ...
                        <div className='space-y-2'>
                            <div className='flex items-center justify-between'>
                                ...
                                <button onClick={()=> generatDescription(index)} disabled={generatingIndex === index || !experience.position || !experience.company} className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
